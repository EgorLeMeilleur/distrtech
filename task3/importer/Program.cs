...
namespace MusicDataImporter
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            logger.LogInformation("Starting Music Data Importer Service");
            
            var configuration = new ConfigurationBuilder()
                .SetBasePath(Directory.GetCurrentDirectory())
                .AddJsonFile("importer_config.json", optional: false, reloadOnChange: true)
                .Build();
            
            var serverCert = File.ReadAllText(configuration["Security:ServerCert"]);
            var serverKey = File.ReadAllText(configuration["Security:ServerKey"]);
            var caCert = File.ReadAllText(configuration["Security:CaCert"]);
            ...
            var port = int.Parse(configuration["Server:Port"]);
            var server = new Server
            {
                Services = { DataImporter.BindService(new DataImporterService(configuration, loggerFactory)) },
                Ports = { new ServerPort(host, port, credentials) }
            };
            
            server.Start();
            
            logger.LogInformation($"Server listening on {host}:{port}");
    ...
    public class DataImporterService : DataImporter.DataImporterBase
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<DataImporterService> _logger;
        
        public DataImporterService(IConfiguration configuration, ILoggerFactory loggerFactory)
        {
            _configuration = configuration;
            _logger = loggerFactory.CreateLogger<DataImporterService>();
        }
