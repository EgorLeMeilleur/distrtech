...
from proto.control_pb2_grpc import TempControlServicer, add_TempControlServicer_to_server
from proto.control_pb2 import Empty

SERVICE_NAME = 'temp_client'
QUEUE_NAME = 'temp_requests'
GRPC_WORKERS = 2

min_temp = 10.0
max_temp = 35.0

class TempClientService(TempControlServicer):
    def SetRange(self, request, context):
        global min_temp, max_temp
        min_temp = request.min
        max_temp = request.max
        print(f"gRPC SetRange received: min={min_temp}, max={max_temp}")
        return Empty()
...
def grpc_server_loop(host, port):
    while True:
        try:
            ...
            add_TempControlServicer_to_server(TempClientService(), server)
    ...
    try:
        while True:
            ...
            try:
                ...
                print(f"Sent temp={value:.2f} at ts={ts}")
            except Exception as e:
